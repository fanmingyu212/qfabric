...
under the same names as the argument names.
...
under the same names as the parameter names.

The following arguments are required by any device class:

* ``resource``: Path to the AWG device.
* ``principal_device``: Whether the device controls other devices via triggers. For multiple AWG devices controlled by qFabric, only one device can be the principal device. The other devices should be triggered by the principal device.
